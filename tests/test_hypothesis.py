...
position_strategy = st.builds(
    ...
    summary=st.one_of(st.none(), st.text(min_size=10, max_size=200)),
)
...
resume_strategy = st.builds(
    Resume,
    name=name_str,
    title_line=st.one_of(st.none(), st.text(min_size=3, max_size=60)),
    theme=st.sampled_from(list(Theme)),
    page_mode=st.sampled_from(list(PageMode)),
    crafted_footer=st.booleans(),
    executive_profile=st.one_of(st.none(), st.text(min_size=10, max_size=500)),
    core_expertise=st.one_of(st.none(), st.text(min_size=10, max_size=300)),
    companies=st.lists(company_strategy, min_size=0, max_size=6),
    technical_skills=st.lists(
        st.builds(SkillCategory,
                  category=st.text(min_size=3, max_size=30),
                  skills=st.text(min_size=5, max_size=200)),
        min_size=0, max_size=6,
    ),
    education=st.lists(
        st.builds(Education,
                  institution=st.text(min_size=5, max_size=50),
                  degree=st.one_of(st.none(), st.text(min_size=5, max_size=60))),
...
def test_json_schema_has_required_fields(resume):
    ...
    if resume.name:
        # name can be None in JSON resume schema
        pass  # validated via test_all_formats_contain_name


@given(resume_strategy)
@settings(max_examples=5, deadline=None)
def test_json_no_markdown_leak(resume):
    """JSON output must not contain **bold** or *italic* markdown."""
    data = resume.to_json_resume()
    raw = json.dumps(data)
    assert "**" not in raw, f"Markdown ** found in JSON output"
...
def test_txt_no_formatting_markers(resume):
    """TXT output should not contain ** or * formatting markers."""
    tmp = Path(tempfile.mkdtemp())
    try:
        TxtRenderer().render(resume, tmp / "resume.txt")
        text = (tmp / "resume.txt").read_text()
...
@given(resume_strategy)
@settings(max_examples=20, deadline=30000)  # Fewer: DOCX is slower
